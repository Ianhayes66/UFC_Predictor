...
class BacktestResult:
    ...
    def save(self) -> Path:
        ...
        pd.Series({"roi": self.roi, "bets": self.bets}).to_json(BACKTEST_PATH, indent=2)
        return BACKTEST_PATH


def run_backtest(probabilities: Iterable[float], outcomes: Iterable[int], prices: Iterable[float]) -> BacktestResult:
...
def backtest() -> BacktestResult:
    ...
    result = run_backtest(df["probability"], np.random.binomial(1, df["probability"]), 1 / df["probability"].clip(lower=0.05))
    result.save()
    return result
